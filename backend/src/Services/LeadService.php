...
final class LeadService
{
    ...
    public function create(array $data): array
    {
        ...
        $duplicate = $this->pdo->prepare('SELECT protocol FROM leads WHERE email=:email AND phone=:phone AND description=:description AND created_at>=:cutoff AND deleted_at IS NULL ORDER BY id DESC LIMIT 1');
        $duplicate->execute(['email'=>$email,'phone'=>$phone,'description'=>$description,'cutoff'=>date('Y-m-d H:i:s', time() - 600)]);
        $existing = $duplicate->fetchColumn();
        if (is_string($existing) && $existing !== '') return ['protocol'=>$existing];
        ...
        $stmt->execute(['protocol'=>$protocol,'name'=>trim((string)$data['name']),'company'=>self::nullable($data['company']??null),'cnpj'=>self::nullable($data['cnpj']??null),'email'=>$email,'phone'=>$phone,'whatsapp'=>self::nullable($data['whatsapp']??null),'city'=>self::nullable($data['city']??null),'state'=>self::nullable($data['state']??null),'segment_id'=>self::intOrNull($data['segment_id']??null),'product_id'=>self::intOrNull($data['product_id']??null),'voltage'=>self::nullable($data['voltage']??null),'power'=>self::nullable($data['power']??null),'quantity'=>max(1,(int)($data['quantity']??1)),'desired_deadline'=>self::nullable($data['desired_deadline']??null),'description'=>$description]);
