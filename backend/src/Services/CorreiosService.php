...
final class CorreiosService
{
    ...
    public function quote(string $destinationCep): array
    {
        ...
        $configuration = [
            'destination'=>$cep,
            'origin'=>Env::get('SHIPPING_ORIGIN_CEP'),
            'weight'=>Env::get('SHIPPING_WEIGHT_GRAMS'),
            'length'=>Env::get('SHIPPING_LENGTH_CM'),
            'width'=>Env::get('SHIPPING_WIDTH_CM'),
            'height'=>Env::get('SHIPPING_HEIGHT_CM'),
            'services'=>array_keys($this->serviceCodes()),
        ];
        $key = hash('sha256', json_encode($configuration, JSON_THROW_ON_ERROR));
        return FileCache::remember('shipping', $key, 300, fn(): array => ExternalRequestGuard::run(fn(): array => $this->fetchQuote($cep)));
    }

    /** @return array{cep:string,address:string,city:string,uf:string,options:list<array{service:string,code:string,price:string,days:int}>} */
    private function fetchQuote(string $cep): array
    {
        $token = $this->token();
        $addressData = $this->request('GET', '/cep/v2/enderecos/' . $cep, $token);
        $options = [];
        foreach ($this->serviceCodes() as $code => $service) {
            $query = http_build_query(['cepDestino'=>$cep,'cepOrigem'=>Env::get('SHIPPING_ORIGIN_CEP'),'psObjeto'=>Env::get('SHIPPING_WEIGHT_GRAMS'),'tpObjeto'=>'2','comprimento'=>Env::get('SHIPPING_LENGTH_CM'),'largura'=>Env::get('SHIPPING_WIDTH_CM'),'altura'=>Env::get('SHIPPING_HEIGHT_CM')]);
            $price = $this->request('GET', "/preco/v1/nacional/{$code}?{$query}", $token);
            $deadline = $this->request('GET', "/prazo/v1/nacional/{$code}?" . http_build_query(['cepOrigem'=>Env::get('SHIPPING_ORIGIN_CEP'),'cepDestino'=>$cep]), $token);
            if (!isset($price['pcFinal'])) continue;
            $options[] = ['service'=>$service,'code'=>$code,'price'=>(string)$price['pcFinal'],'days'=>(int)($deadline['prazoEntrega'] ?? 0)];
        }
    ...
    private function token(): string
    {
        ...
        $body = ['numero'=>Env::get('CORREIOS_POSTING_CARD'),'contrato'=>Env::get('CORREIOS_CONTRACT')];
    ...
    {
        $codes = array_slice(array_values(array_unique(array_filter(array_map('trim', explode(',', Env::get('CORREIOS_SERVICE_CODES', '03298,03220') ?? '')), static fn(string $code): bool => preg_match('/^\d{5}$/', $code) === 1))), 0, 4);
