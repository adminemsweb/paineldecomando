...
const adminLinks = [['/admin/dashboard', 'Visão geral'], ['/admin/produtos', 'Produtos'], ['/admin/categorias', 'Categorias'], ['/admin/segmentos', 'Segmentos'], ['/admin/servicos', 'Serviços'], ['/admin/projetos', 'Projetos'], ['/admin/posts', 'Posts'], ['/admin/leads', 'Leads'], ['/admin/configuracoes', 'Configurações']];
...
export function AdminLayout() {
  ...
  return <div className="admin-shell">
    <aside className={`admin-sidebar${menuOpen ? ' admin-sidebar--open' : ''}`}>
      ...
      <nav aria-label="Administração">{adminLinks.map(([to, label]) => <NavLink key={to} to={to} onClick={() => setMenuOpen(false)}>{label}</NavLink>)}</nav>
