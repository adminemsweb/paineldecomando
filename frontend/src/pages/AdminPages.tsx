...
export function AdminDashboardPage() {
  ...
  const deviceTotal = Math.max(1, analytics?.devices.reduce((total, item) => total + Number(item.visitors), 0) ?? 0);
  const pagesPerVisitor = summary?.visitors ? (summary.page_views / summary.visitors).toFixed(1).replace('.', ',') : '0';
  const metrics = [
    { label:'Visitantes', value:summary?.visitors ?? 0, detail:'pessoas diferentes', icon:'user' as const, tone:'blue' },
    { label:'Acessos', value:summary?.page_views ?? 0, detail:'páginas abertas', icon:'clipboard' as const, tone:'yellow' },
    { label:'Painéis vistos', value:summary?.product_views ?? 0, detail:'produtos visualizados', icon:'search' as const, tone:'purple' },
    { label:'Buscas', value:summary?.searches ?? 0, detail:'pesquisas no catálogo', icon:'tag' as const, tone:'orange' },
    { label:'Contatos', value:summary?.conversions ?? 0, detail:`${summary?.conversion_rate ?? 0}% de conversão`, icon:'whatsapp' as const, tone:'green' },
  ];
  return <>
    ...
    {loading ? <div className="admin-empty" role="status">Carregando métricas…</div> : <>
      <div className="admin-cards admin-cards--analytics">
        {metrics.map(metric => <article className={`analytics-metric analytics-metric--${metric.tone}`} key={metric.label}><div><span>{metric.label}</span><i><Icon name={metric.icon} size={18}/></i></div><strong>{metric.value}</strong><small>{metric.detail}</small></article>)}
      </div>
      <div className="analytics-summary"><strong>Resumo do período</strong><span>{summary?.visitors ?? 0} visitantes fizeram {summary?.page_views ?? 0} acessos, visualizaram {summary?.product_views ?? 0} painéis e iniciaram {summary?.conversions ?? 0} contatos.</span></div>
      <div className="analytics-layout">
        <section className="admin-panel analytics-panel analytics-panel--traffic"><header><div><span>Tráfego</span><h2>Movimento do site</h2><p>Compare todos os acessos com as visualizações de produtos.</p></div><div className="analytics-legend"><small><i className="is-pages"/>Acessos</small><small><i className="is-products"/>Painéis vistos</small></div></header>{Number(summary?.page_views ?? 0) > 0 ? <div className="analytics-graph"><svg viewBox="0 0 1000 200" preserveAspectRatio="none" role="img" aria-label="Gráfico de acessos e painéis visualizados por dia"><defs><linearGradient id="analyticsArea" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#17a6a1" stopOpacity=".28"/><stop offset="1" stopColor="#17a6a1" stopOpacity=".02"/></linearGradient></defs>{[35,83,132,180].map(y => <line key={y} x1="0" y1={y} x2="1000" y2={y} className="analytics-grid-line"/>)}<polygon points={`0,180 ${pagePoints} 1000,180`} fill="url(#analyticsArea)"/><polyline points={pagePoints} className="analytics-line analytics-line--pages"/><polyline points={productPoints} className="analytics-line analytics-line--products"/></svg><div className="analytics-axis">{labelIndexes.map(index => <small key={chartDays[index].date}>{new Date(`${chartDays[index].date}T12:00:00`).toLocaleDateString('pt-BR', { day:'2-digit', month:'short' }).replace('.', '')}</small>)}</div></div> : <div className="analytics-empty-state"><Icon name="clipboard" size={26}/><strong>Ainda não há acessos neste período</strong><span>As visitas começarão a formar o gráfico automaticamente.</span></div>}</section>
        <section className="admin-panel analytics-panel analytics-panel--overview"><header><div><span>Leitura rápida</span><h2>O que os números mostram</h2><p>Indicadores simples para acompanhar o desempenho.</p></div></header><div className="analytics-highlights"><div><span>Páginas por visitante</span><strong>{pagesPerVisitor}</strong><small>Média de navegação</small></div><div><span>WhatsApp</span><strong>{summary?.whatsapp_clicks ?? 0}</strong><small>Cliques para atendimento</small></div><div><span>Orçamentos</span><strong>{summary?.quote_clicks ?? 0}</strong><small>Pedidos iniciados</small></div></div></section>
        ...
        <section className="admin-panel analytics-panel analytics-panel--devices"><header><div><span>Dispositivos</span><h2>Como acessam</h2><p>Origem dos visitantes por tamanho de tela.</p></div></header><div className="analytics-devices">{analytics?.devices.length ? analytics.devices.map(device => { const label = device.device === 'mobile' ? 'Celular' : device.device === 'tablet' ? 'Tablet' : device.device === 'desktop' ? 'Computador' : 'Outro'; const percentage = Math.round((Number(device.visitors) / deviceTotal) * 100); return <div key={device.device}><div><span>{label}</span><small>{percentage}% dos visitantes</small></div><strong>{device.visitors}</strong><i><b style={{ width:`${percentage}%` }}/></i></div>; }) : <div className="analytics-list-empty">Sem dados ainda.</div>}</div></section>
